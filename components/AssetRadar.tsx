...
function compute(a:any){
  ...
  const provenance = a.description?.toLowerCase().match(/(certificate|coac|coa|sotheby|christie|museum|royal|documented)/) ? 90 : 70
  return [
...
export default function AssetRadar({ asset, height=220 }:{asset:any,height?:number}){
  const data = useMemo(()=>compute(asset), [asset])
  return (
    <div style={{height}} className="radar-card">
